...
- **Break reminders** — an optional recurring nudge ("stand up and move") every 30/45/60 min in any mode, with a custom label and an optional active-hours window. When it's time the bell pill flags **Break now** (with your message scrolling beneath it) and pulses, and the dropdown auto-opens (chime + browser notification too): **Take a break** starts a configurable timer (5/10/15 min), dismisses the dropdown and shrinks the pill to a compact break clock, pausing the next-break countdown until it finishes — or **Skip** restarts the interval right away. A quick **pause/resume** button lives in the pill so you can hold the next-break countdown without opening the dropdown, and the pill mirrors the next-break / paused / on-break time on every screen (notifications reach you even when Flipit isn't the active tab, with a graceful chime fallback)
